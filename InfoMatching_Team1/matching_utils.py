...
def match_resumes_to_jobs(
    ...
    try:
        ...
        # Get job qualifications and Experience domain
        job_ids = job_df['sql_id'].tolist()
        placeholders = ', '.join(['?'] * len(job_ids))
        query = f"SELECT id, minimum_academic_qualification, experience_domain FROM job_description WHERE id IN ({placeholders})"
...
def match_new_resumes_to_jobs(new_resumes, model_name="bge-m3", csv_path="search_results.csv"):
    ...
    connection = sqlite3.connect(db_path)
    cursor = connection.cursor()
    # Get job qualifications and Experience domain for HARD CONSTRAINTS
    job_ids = job_df['sql_id'].tolist()
    placeholders = ', '.join(['?'] * len(job_ids))
    query = f"SELECT id, minimum_academic_qualification, experience_domain FROM job_description WHERE id IN ({placeholders})"
    cursor.execute(query, job_ids)
